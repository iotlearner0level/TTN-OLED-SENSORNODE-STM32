...
uint8_t Adafruit_BMP085::read8(uint8_t a) {
  ...
  Tire.beginTransmission(BMP085_I2CADDR); // start transmission to device 
#if (ARDUINO >= 100)
  Tire.write(a); // sends register address to read from
#else
  Tire.send(a); // sends register address to read from
#endif
  Tire.endTransmission(); // end transmission
  
  Tire.beginTransmission(BMP085_I2CADDR); // start transmission to device 
  Tire.requestFrom(BMP085_I2CADDR, 1);// send data n-bytes read
#if (ARDUINO >= 100)
  ret = Tire.read(); // receive DATA
#else
  ret = Tire.receive(); // receive DATA
#endif
  Tire.endTransmission(); // end transmission
...
uint16_t Adafruit_BMP085::read16(uint8_t a) {
  ...
  Tire.beginTransmission(BMP085_I2CADDR); // start transmission to device 
  Tire.requestFrom(BMP085_I2CADDR, 2);// send data n-bytes read
#if (ARDUINO >= 100)
  ret = Tire.read(); // receive DATA
  ret <<= 8;
  ret |= Tire.read(); // receive DATA
...
#if (ARDUINO >= 100)
  Tire.write(a); // sends register address to read from
  Tire.write(d);  // write data
#else
  Tire.send(a); // sends register address to read from
  Tire.send(d);  // write data
#endif
  Tire.endTransmission(); // end transmission
}
